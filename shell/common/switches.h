...
DEF_SWITCHES_START
DEF_SWITCH(AotSharedLibraryName, "aot-shared-library-name",
           "Name of the *.so containing AOT compiled Dart assets.")
DEF_SWITCH(AotVMServiceSharedLibraryName, "aot-vmservice-shared-library-name",
           "Name of the *.so containing AOT compiled Dart assets for "
           "launching the service isolate.")
DEF_SWITCH(SnapshotAssetPath, "snapshot-asset-path",
...
DEF_SWITCH(FlutterAssetsDir, "flutter-assets-dir",
...
DEF_SWITCH(
    ...
    "Fuchsia.")
DEF_SWITCH(TraceToFile, "trace-to-file",
...
DEF_SWITCH(RunForever, "run-forever",
           "In non-interactive mode, keep the shell running after the Dart "
           "script has completed.")
DEF_SWITCH(DisableDartAsserts, "disable-dart-asserts",
           ...
           "or DBC).")
DEF_SWITCH(EnableSerialGC, "enable-serial-gc",
...
DEF_SWITCH(DisallowInsecureConnections, "disallow-insecure-connections",
           "By default, dart:io allows all socket connections. If this switch "
           "is set, all insecure connections are rejected.")
DEF_SWITCH(DomainNetworkPolicy, "domain-network-policy",
...
DEF_SWITCH(
    ForceMultithreading, "force-multithreading",
...
DEF_SWITCH(EnableOpenGLGPUTracing, "enable-opengl-gpu-tracing",
           "Enable tracing of GPU execution time when using the Impeller "
           "OpenGLES backend.")
DEF_SWITCH(LeakVM, "leak-vm",
...
DEF_SWITCH(EnableEmbedderAPI, "enable-embedder-api",
           "Enable the embedder api. Defaults to false. iOS only.")
DEF_SWITCH(ForceSimulatorRun, "force-simulator-run",
